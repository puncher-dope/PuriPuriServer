...
export class CardsWaitersService {
  constructor(
    @InjectModel(CardsWaiters.name)
    private cardsWaitersModel: Model<CardsWaiters>,
  ) {}

  //CREATE
  async createCardWaiters(
    body: Partial<CardsForWaiters>,
  ): Promise<CardsWaiters[]> {
  ...
  async updateCard(
    id: string,
    body: Partial<CardsForWaiters>,
  ): Promise<CardsWaiters[] | null> {
    await this.cardsWaitersModel
